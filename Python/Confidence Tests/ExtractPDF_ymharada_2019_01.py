...
OutputFolder        = r'C:\Users\ymharada\UW\Engineering Services GIS - GIS\Confidence Test Program'
...
def lastFunction():
  #leave this value here. It is the exception line
    x = -99
...
#the end of def lastFunction
process = multiprocessing.Process(target=lastFunction)
process.start()
process.join()
x = 10
#print statement optional #print "The last function has finished."


#This is an if/else statement that evaluates whether the statement is true or false. This takes a while to run.
  #if this statement is true, the code is sucessful 
if (x == 10):
    yesp = 'The python code has run sucessfully and this csv file serves as proof of thus!'
    #print statement optional #print yesp
    with open(rtime + '_Success_ConfidenceTest.csv', 'ab+') as csvfile:
     writer = csv.writer(csvfile)
     writer.writerow(["Message", "Date"])
     writer.writerow([yesp, rtime])
     raise SystemExit(0)
  #if this statement is not true, the code is unsucessful.
elif x != 10:
    failp = "The code failed. The error message is as follows:"+ repr(err) + "."
    #print statement optional #print failp
    with open(rtime + '_Failed_ConfidenceTest.csv', 'ab+') as csvfile:
     writer = csv.writer(csvfile)
     writer.writerow(["Message", "Date"])
     writer.writerow([failp, rtime])
raise SystemExit(0)
