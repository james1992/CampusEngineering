...
############################################################################# 
### Description: This script is designed to sync changes from replicas that
### are stored in the IAMUW_REPLICATION database on the pub server.  Changes
### are synced to parent datasets in IAMUW server, these changes are then 
### compressed to base tables and the statistics are updated for every table
### in the given database that has been registered with the ESRI platform.
### The script will be set to run automatically every Sunday afternoon.
...
def main(DbConnections, CEOReplicas, MCReplicas, CeoObjects, McObjects):
    #Sync the Replicas
    SyncReplicaDatasets(CEOReplicas, MCReplicas)

    #Compress IAMUW Databases
    DatabaseCompression(DbConnections)
    
    #Compress edits in CEO and MAC databases
    CeoCompressToBase(CeoObjects)
    McCompressToBase(McObjects)
    
    #Analyze Database Table Statistics
    AnalyzeDatabaseStatistics(DbConnections)
...
if __name__ == "__main__":
    main(DatabaseConnections, CEODatabaseReplicas, MCDatabaseReplicas, CeoDatabaseObjects, McDatabaseObjects)
