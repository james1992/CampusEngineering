...
def CreateDeliverables(FeatureClass, SewerLidMatches, StormLidMatches):
    ...
    WrongFeatureClass = ReturnMislabeledManholes(Date, FeatureClass)

    # Populate the Lid Match and Distance fields with the values from the
    # analysis that was performed in this script.
    UpdateFields = ["REL_GUID", "MatchedLid", "Distance"]
    CreateUnmatchFields(WrongFeatureClass)
    WrongLidUpdateCursor(WrongFeatureClass, UpdateFields, SewerLidMatches)
    WrongLidUpdateCursor(WrongFeatureClass, UpdateFields, StormLidMatches)
...
def ReturnMislabeledManholes(DateToday, Dataset):
    '''
    ...
    '''
    NewFeatureClass = "Wrong" + '_' + DateToday
...
def CreateUnmatchFields(WrongLidsDataset):
    ...
    '''
    arcpy.AddField_management(WrongLidsDataset, "MatchedLid", "TEXT", "", "", 100)
    ...
    with arcpy.da.UpdateCursor(WrongLidsDataset, Fields) as Cursor:
        for Row in Cursor:
            for Entry in LidMatches:
                if Row[0] == Entry:
                    Row[1] = LidMatches[Entry][0]
                    Row[2] = LidMatches[Entry][1]
                    Cursor.updateRow(Row)
                else:
                    pass                    
    del Cursor         
